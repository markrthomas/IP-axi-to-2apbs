...
// Parity with test/tb_axi4_to_apb4_2x_simple_ws.v - APB read stall depth: BRIDGE_READ_WS (+define or default 2).
`ifndef BRIDGE_READ_WS
`define BRIDGE_READ_WS 2
`endif

module tb_uvm_simple_ws;
  ...
  localparam int unsigned READ_WS = `BRIDGE_READ_WS;
  ...
  initial begin
    uvm_config_db #(bridge_stimulus_pkg::v_axi_if_64_t)::set(null, "uvm_test_top",
                                                            "axi_vif", axi_if);
    uvm_config_db #(int unsigned)::set(null, "uvm_test_top", "read_wait_cycles", READ_WS);
    uvm_config_db #(
        virtual apb_mon_if #(
            .ADDR_WIDTH(ADDR_WIDTH),
            .DATA_WIDTH(DATA_WIDTH)
    ...
    run_test("test_bridge_simple_ws");
  end
